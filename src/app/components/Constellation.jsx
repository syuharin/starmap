    ...
    <group position={position}>
      ...
      {name && (
        <Html distanceFactor={10}>
          <div style={{ color: "white", fontSize: "12px" }}>{name}</div>
        </Html>
      )}
    </group>
  );
...
const Star2D = ({ position, magnitude, name }) => {
  ...
  useEffect(() => {
    ...
    if (name) {
      ...
      ctx.fillText(
        name,
        position.x + canvas.width / 2 + size + 2,
        -position.y + canvas.height / 2,
      );
    }
  ...
  useEffect(() => {
    if (focusedObject) {
      ...
      if (focusedObject.type === "star") {
        ...
        objectPosition.set(
          radius *
            Math.cos(THREE.MathUtils.degToRad(declination)) *
            Math.cos(THREE.MathUtils.degToRad(right_ascension)),
          radius * Math.sin(THREE.MathUtils.degToRad(declination)),
          -radius *
            Math.cos(THREE.MathUtils.degToRad(declination)) *
            Math.sin(THREE.MathUtils.degToRad(right_ascension)),
        );
      } else if (focusedObject.type === "constellation") {
        const right_ascension = parseFloat(
          focusedObject.right_ascension_center,
        );
        ...
        if (!isNaN(right_ascension) && !isNaN(declination)) {
          ...
          console.error(
            "Invalid constellation center coordinates:",
            focusedObject,
          );
          return;
        }
      ...
      // 方位円の中心から天体への方向を計算
      // const direction = objectPosition.normalize(); // Unused - Removed
      ...
      <group>
        ...
        {constellationData.constellations.map((constellation) =>
          constellation.stars.map((star) => {
            ...
            // 方位角の計算
            const cosA =
              (Math.sin(dec) - Math.sin(altitude) * Math.sin(latitude)) /
              (Math.cos(altitude) * Math.cos(latitude));
            const azimuth =
              Math.sin(hourAngle) > 0
                ? 2 * Math.PI - Math.acos(cosA)
                : Math.acos(cosA);
        ...
        {constellationData.constellations.map((constellation) =>
          constellation.lines.map((line) => {
            ...
            const azimuth1 =
              Math.sin(hourAngle1) > 0
                ? 2 * Math.PI - Math.acos(clampedCosA1)
                : Math.acos(clampedCosA1);
            ...
            const azimuth2 =
              Math.sin(hourAngle2) > 0
                ? 2 * Math.PI - Math.acos(clampedCosA2)
                : Math.acos(clampedCosA2);
      ...
      >
        {constellationData.constellations.map((constellation) =>
          constellation.stars.map((star) => {
            ...
            const position = {
              x:
                Math.cos(THREE.MathUtils.degToRad(star.right_ascension)) *
                scale,
