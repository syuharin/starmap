...
const validateApiResponse = (data) => {
  ...
  data.constellations.forEach((constellation, index) => {
    ...
    constellation.stars.forEach((star, starIndex) => {
      if (
        !star ||
        typeof star.right_ascension !== "number" ||
        typeof star.declination !== "number"
      ) {
        throw new Error(
          `星の座標データが不正です (constellation: ${index}, star: ${starIndex})`,
        );
      }
      if (typeof star.magnitude !== "number") {
        throw new Error(
          `星の等級データが不正です (constellation: ${index}, star: ${starIndex})`,
        );
      }
    });
    ...
    constellation.lines.forEach((line, lineIndex) => {
      if (!line || !line.star1 || !line.star2) {
        throw new Error(
          `星座線のデータが不正です (constellation: ${index}, line: ${lineIndex})`,
        );
      }
    });
  });
...
const transformConstellationData = (apiData) => {
  ...
  apiData.constellations.forEach((constellation) => {
    ...
    constellation.lines.forEach((line) => {
      const start = convertToCartesian(
        line.star1.right_ascension,
        line.star1.declination,
      );
      const end = convertToCartesian(
        line.star2.right_ascension,
        line.star2.declination,
      );
...
const validateTransformedData = (data) => {
  ...
  data.lines.forEach((line, index) => {
    ...
    if (
      !line.start ||
      typeof line.start !== "object" ||
      !line.end ||
      typeof line.end !== "object"
    ) {
    ...
    ["start", "end"].forEach((point) => {
      ["x", "y", "z"].forEach((coord) => {
        if (typeof line[point][coord] !== "number") {
          throw new Error(
            `変換後の星座線の座標値が不正です (index: ${index}, point: ${point}, coord: ${coord})`,
          );
        }
      });
    });
  ...
  data.stars.forEach((star, index) => {
    ...
    ["x", "y", "z"].forEach((coord) => {
      if (typeof star[coord] !== "number") {
        throw new Error(
          `変換後の星の座標値が不正です (index: ${index}, coord: ${coord})`,
        );
      }
    });
    if (typeof star.magnitude !== "number") {
