...
const fetchWithRetry = async (url, options, retries = 3, delay = 1000) => {
  for (let i = 0; i < retries; i++) {
    try {
      ...
      console.warn(`[fetchWithRetry] Attempt ${i + 1} failed for ${url} with status ${response.status}. Retrying in ${delay / 1000}s...`);
    } catch (error) {
      // ネットワークエラーなど fetch 自体の失敗
      console.warn(`[fetchWithRetry] Attempt ${i + 1} failed for ${url} with error: ${error}. Retrying in ${delay / 1000}s...`);
    }
  ...
  console.error(`[fetchWithRetry] All ${retries} attempts failed for ${url}. Returning the result of the last attempt.`);
...
export const fetchStars = async (latitude, longitude, altitude = 0, datetime = null) => {
    ...
    if (!response.ok) {
        throw new Error(`星のデータの取得に失敗しました (Status: ${response.status})`);
    }
...
export const fetchConstellations = async (selectedDate) => {
    ...
    const response = await fetchWithRetry(`${API_BASE_URL}/constellations?${params}`);
    if (!response.ok) {
        throw new Error(`星座データの取得に失敗しました (Status: ${response.status})`);
    }
    return await response.json();
};
