...
def init_database():
    ...
    try:
        ...
        # マッピング用辞書
        constellation_map: Dict[str, int] = {}  # abbreviation -> id
        star_map: Dict[int, int] = {}  # hip_number -> id

        # 2. 星座データをロード
        _load_constellations(db, constellation_map)

        # 3. 星データをロード
        _load_stars(db, constellation_map, star_map)
