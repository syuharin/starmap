...
if not DATABASE_URL or not DATABASE_URL.startswith("postgresql"):
    ...
    print("環境変数DATABASE_URLを設定してください。例: postgresql://user:password@localhost:5432/starmap")
    sys.exit(1)
...
def migrate_data():
    ...
    try:
        ...
        for star in stars:
            new_star = Star(
                name=star['name'],
                common_name_jp=star['common_name_jp'] if 'common_name_jp' in star.keys() else None,
                bayer_designation=star['bayer_designation'] if 'bayer_designation' in star.keys() else None,
